...
/*
 Aspect(애스팩트) : AOP의 단위가 되는 횡단 관심사
  - 횡단 관심사(Cross-Cutting Concern) : 핵심(core) 비지니스 로직(삼겹살구어먹기, 빵또아의 아이스크림)과
...
public class ServiceLoggerAdvice {
	...
	public void logReturnion(JoinPoint jp) {
		log.info("logReturning");
		
		log.info("logReturnint: "+jp.getSignature());
	}
	...
	//After 어드바이스
	// 조인포인트 kr.or.ddit.service.BoardService register(BoardVO) 완료 후 실행
	// 예외 발생이 되더라도 항상 실행 됨
	...
	//횡단관심사 - 부수적, 포인트컷 대상
	//스프링프레임워크가 컨트롤 하고 있는 비지니스로직 호출을 가로챔. 책임이 around 어드바이스로 전가됨
